...
+ (SecKeyRef)addPublicKey:(NSString *)key {
	...
	data = [RSAOC stripPublicKeyHeader:data];
	if (!data) {
		return nil;
	}
...
+ (SecKeyRef)addPrivateKey:(NSString *)key {
	...
	// This will be base64 encoded, decode it.
	NSData *data = base64_decode(key);
	data = [RSAOC stripPrivateKeyHeader:data];
	if (!data) {
		return nil;
	}
...
+ (NSString *)encryptString:(NSString *)str privateKey:(NSString *)privKey {
	NSData *data = [RSAOC encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] privateKey:privKey];
...
+ (NSData *)encryptData:(NSData *)data privateKey:(NSString *)privKey {
	...
	SecKeyRef keyRef = [RSAOC addPrivateKey:privKey];
	if (!keyRef) {
		return nil;
	}
	return [RSAOC encryptData:data withKeyRef:keyRef];
}
...
+ (NSString *)decryptString:(NSString *)str privateKey:(NSString *)privKey {
	NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
	data = [RSAOC decryptData:data privateKey:privKey];
	NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
	return ret;
}
...
+ (NSData *)decryptData:(NSData *)data privateKey:(NSString *)privKey {
	...
	return [RSAOC decryptData:data withKeyRef:keyRef];
}
...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey {
	NSData *data = [RSAOC encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey];
	NSString *ret = base64_encode_data(data);
	return ret;
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey {
	...
	SecKeyRef keyRef = [RSAOC addPublicKey:pubKey];
	if (!keyRef) {
		return nil;
	}
	return [RSAOC encryptData:data withKeyRef:keyRef];
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey {
	NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
	data = [RSAOC decryptData:data publicKey:pubKey];
	NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
	return ret;
}
...
+ (NSString *)signString:(NSString *)plainText withPrivatekeyRef:(SecKeyRef)private_keyRef {
	...
	sanityCheck = SecKeyRawSign(private_keyRef,
	                            kTypeOfSigPadding,
	                            (const uint8_t *)[[RSAOC getHashBytes:plainTextBytes] bytes],
	                            kChosenDigestLength,
	                            (uint8_t *)signedBytes,
	                            &signedBytesSize);
...
+ (SecKeyRef)getPrivateKeyfromFile:(NSString *)p12file_name password:(NSString *)pwd_string {
	...
	SecKeyRef privateKeyRef_result = [RSAOC getPrivateKeyRefrenceFromData:p12Data password:pwd_string];
...
+ (NSString *)signString:(NSString *)plain_string withPrivatekey:(NSString *)privKey {
	...
	NSString *base64String = [RSAOC signString:plain_string withPrivatekeyRef:keyRef];
	return [RSAOC urlEncodedString:base64String];
}

+ (NSString *)signString:(NSString *)plain_string byP12File:(NSString *)fileName password:(NSString *)pwd_string {
	SecKeyRef keyRef = [RSAOC getPrivateKeyfromFile:fileName password:pwd_string];
	if (!keyRef) {
		return nil;
	}
	NSString *base64String = [RSAOC signString:plain_string withPrivatekeyRef:keyRef];
